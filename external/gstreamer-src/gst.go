...
func CreatePipeline(codecName string, tracks []*webrtc.TrackLocalStaticSample, pipelineSrc string) *Pipeline {
	...
	case "h264":
		pipelineStr = pipelineSrc + " ! video/x-raw,width=1280,height=720,format=I420,framerate=30/1 ! nvh264enc bitrate=2000 rc-mode=cbr gop-size=30 bframes=0 qos=true preset=low-latency-hq ! video/x-h264,profile=high,stream-format=byte-stream ! " + pipelineStr
		clockRate = videoClockRate
...
func goHandlePipelineBuffer(buffer unsafe.Pointer, bufferLen C.int, duration C.int, pipelineID C.int) {
	...
	if ok {
		for _, t := range pipeline.tracks {
			fmt.Printf("buffer : ", bufferLen, "duration", duration)
